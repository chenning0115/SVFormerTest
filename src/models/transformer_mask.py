    ...
    def encoder_block(self, x):
        ...
        if self.use_mask: # 使用mask策略
            if self.training: 
                ...
                # mask serving
                mlp_output_list = []
                for cur_mask in self.mask_list:
                    x_pixel, x_center_list = self.local_trans_pixel(x_pixel, mask=cur_mask.to(device))
                    logit_x = x_center_list[-1]
                    x_output = self.classifier_mlp(logit_x) # [batch, class_num]
                    mlp_output_list.append(x_output)
                logit_x = torch.stack(mlp_output_list,dim=1) #[batch, mask_num, class_num]
                logit_p = torch.mean(torch.softmax(logit_x, dim=-1), 1)
                return logit_p
        else:
            cur_mask = None
            x_pixel, x_center_list = self.local_trans_pixel(x_pixel, mask=cur_mask)
